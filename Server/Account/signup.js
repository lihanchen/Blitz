	...
	try{
		global.collection.findOne({username:receivedObj.username},function(err,item){
			if (item!=null){
				...
				global.collection.insert({username:receivedObj.username,password:receivedObj.password,email:receivedObj.email,rating:0,hold:{reason:"unverified",unlockCode:unlockCode}});
				//Send verifi email
				ret.success=true;
